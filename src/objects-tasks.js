...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (change !== 0) return false; // has to be exactly 0
...
class Selector {
  ...
  validatePosition(selector) {
    const index = this.order.indexOf(selector);
  ...
  class(value) {
    this.validatePosition('class');
    this.selector += `.${value}`;
    return this;
  }

  attr(value) {
    this.validatePosition('attr');
    this.selector += `[${value}]`;
    return this;
  }

  pseudoClass(value) {
    this.validatePosition('pseudoClass');
    this.selector += `:${value}`;
    return this;
  }

  pseudoElement(value) {
    this.validatePosition('pseudoElement');
